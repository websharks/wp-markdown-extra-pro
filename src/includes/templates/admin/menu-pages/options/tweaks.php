...
<?= $Form->openTag(); ?>
    ...
    <?= $Form->openTable(
        ...
        <?= $Form->selectRow([
            ...
            'note'  => __('A mixed set of helpful tweaks for the professional Markdowner. This disables the <code>convert_smilies</code>, <code>convert_chars</code>, <code>wpautop</code>, <code>shortcode_unautop</code>, and <code>capital_P_dangit</code> filters in WordPress core; i.e., filters that can, at times, lead to quirky surprises when written in Markdown or raw HTML. This also enables Shortcodes in Text Widgets.<br /><br /><em><strong>Note:</strong> With this option enabled, the filters mentioned above are adjusted globally, for all content, excerpts &amp; text widgets, whether they\'re written in Markdown or not. However, they\'re only removed for Posts if you have Markdown enabled for Posts. The same for Text Widgets and Comments; i.e., filters are only removed globally for content types with Markdown enabled.</em>', 'wp-markdown-extra'),
